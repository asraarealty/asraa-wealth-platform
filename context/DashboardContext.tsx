...
async function fetchDashboardFull(): Promise<DashboardOperatingData> {
  const res = await fetcher<any>("/dashboard/full", { raw: true });
  const payload = res?.data && typeof res.data === "object" ? res.data : res;

  const summaryRaw = payload?.summary ?? {};
  const portfolioRaw = payload?.portfolio ?? {};
  const allocationRaw = payload?.allocation ?? {};
  const riskRaw = payload?.risk ?? {};
  ...
  const priorityActionsRaw = asArray<Record<string, unknown>>(
  ...
  const realEstateRaw = portfolioRaw?.real_estate ?? portfolioRaw?.realEstate ?? {};
  const occupied = asNum(realEstateRaw?.occupied, properties.filter((p) => Boolean(p?.tenant_name)).length);
  const totalProperties = properties.length;
  const occupancyPct = totalProperties > 0 ? (occupied / totalProperties) * 100 : asNum(realEstateRaw?.occupancy_pct);
  ...
  const executive: DashboardExecutive = {
    ...
    riskState: asStr(riskRaw?.risk_state ?? riskRaw?.state ?? riskRaw?.level, "Low"),
  };
